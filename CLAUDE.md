...
| **Supervisor Checks** | Partial | Entry screen now has Crane Meeting card + Rescue Kit Checklist card + Coming Soon for pre-lift checklists |
| **Crane Meeting** | Built & Working | Weekly per-site safety meeting accessed from Supervisor Checks. Toolbar: Set up / Generate / View / Reset / Archive. Set Up form with persistent project + 8 multi-line meeting detail textareas (review, incidents, revised methods, future lifts, weather, new methods, lifting equipment, AOB) + next meeting date + creator drawn signature. Active meeting embedded as HTML inline like Daily Briefing. Real-time Signed counter + Attendance modal (Read + Signed lists). Scroll-to-bottom read tracking, drawn signature canvas. Auto-archive at 19:59 Friday + safety-net reset at 20:00 Friday via pg_cron, plus manual Generate. Archive PDF via pdf-lib Edge Function: cover, full meeting content, creator sign-off, attendees signature table last. All text ASCII-sanitised; HTML entities decoded; div content captured; newlines preserved as paragraphs. Archive screen lists all past PDFs |
| **Rescue Kit Checklist** | Built & Working | Per-site weekly tower crane rescue kit verification inside Supervisor Checks. AP / crane_supervisor only. Add form with persistent kit details (contractor, project, serial, location, secured status, harness count + serials, certificates with expiry, seal status). Yes/No toggles for binary fields, free text for the rest, optional individual/company serial number field. Weekly Pending Check section with two sub-areas (awaiting sign / signed this week) — every kit row has Edit + Delete menu always accessible (fixed-position dropdown auto-flips upward for bottom rows, escapes section card clipping). Sign flow: supervisor name + drawn signature → PDF generated via pdf-lib Edge Function with no Pro-lifting branding. Each weekly signing creates a new version (v1, v2, v3…) archived permanently. Re-sign supported. Monday 00:00 weekly rotation via pg_cron. Document reference: {contractor} {project} Rescue Kit Checklist v{N} |
...
The Crane Meeting feature delivers a weekly per-site crane coordination meeting for lifting teams. The `appointed_person` or `crane_supervisor` completes a Set Up form covering the project details and eight multi-line meeting agenda sections. Submitting the form assembles a full HTML meeting document and makes it available to all site operatives to read and sign.

**Weekly lifecycle:**
1. AP or supervisor opens the Set Up form and fills in the meeting details.
2. The completed meeting document is embedded inline on the home screen for all operatives.
3. Operatives scroll to the bottom (read tracking) and tap **Sign Off** to draw their signature.
4. AP or supervisor can monitor attendance via the **Attendance** modal with live Realtime updates.
5. At 19:59 Friday, the meeting is automatically archived via `pg_cron`. A safety-net reset job at 20:00 Friday force-archives any still-active meeting. The AP can also trigger archiving manually via the **Generate** button at any time.

Each site has **one active crane meeting at a time**. Enforced by a partial unique index on `site_id WHERE status = 'active'`. Soft-delete only — no records are ever hard-deleted.

**Persistent settings:** Project, Project No, all eight meeting detail textareas, and Date of Next Meeting are stored in `crane_meeting_settings` (one row per site) and pre-filled for each new meeting. Meeting Date and Meeting Time are entered fresh per meeting. The creator signature is drawn fresh each meeting.

**Entry point:** A Crane Meeting card/button on `app/(appointed-person)/supervisor-checks.tsx` navigates to the Crane Meeting home screen.
...
| Role | Permissions |
|---|---|
| `appointed_person` | Set Up meeting, edit persistent fields, Generate PDF, View archive PDF, Reset active meeting, Archive, view Attendance |
| `crane_supervisor` | Set Up meeting, edit persistent fields, Generate PDF, View archive PDF, Reset active meeting, Archive, view Attendance |
| `crane_operator` | Read active meeting, sign active meeting, view Archive |
| `slinger_signaller` | Read active meeting, sign active meeting, view Archive |
| `subcontractor_admin` | Read active meeting, sign active meeting, view Archive |
...
| Screen | Path | Description |
|---|---|---|
| Home | `/(appointed-person)/crane-meeting/` | Toolbar actions, project info cards, signed counter, embedded meeting document, fixed Sign Off button |
| Set Up | `/(appointed-person)/crane-meeting/setup` | Multi-section form to create or edit the active meeting |
| Sign | `/(appointed-person)/crane-meeting/sign` | Drawn signature modal — presented as a stack screen with `presentation: 'modal'` |
| Attendance | `/(appointed-person)/crane-meeting/attendance` | Who Signed modal showing Read + Signed per operative, live via Supabase Realtime |
| Archive | `/(appointed-person)/crane-meeting/archive` | List of all archived meetings with view/delete actions |

All five routes are registered in `app/(appointed-person)/_layout.tsx`. Entry point is the **Crane Meeting** card on `app/(appointed-person)/supervisor-checks.tsx`.

---

### 11.4 Home Screen Layout

The home screen is the central view for all roles. It refreshes on focus via `useFocusEffect`.

**Top toolbar — action buttons:**
- **Set up the meeting** (AP/supervisor only) — opens Set Up form
- **Generate** (AP/supervisor only) — manually triggers archive PDF generation
- **View** (AP/supervisor only) — opens the latest archived PDF
- **Reset** (AP/supervisor only) — force-archives the active meeting (soft-delete with confirmation)
- **Archive** (visible to all roles) — opens Archive screen showing all past meetings

**Info cards:**
- Project name card and Phase/Project No card displayed side by side below the toolbar.

**Signed counter:**
- `Signed: n` badge updated in real time via Supabase Realtime on `crane_meeting_signatures`.
- **Who Signed** button (AP/supervisor only) opens the Attendance modal.

**Document area:**
- If no active meeting exists: prompt to set up the meeting (AP/supervisor) or message that no meeting has been set up yet (other roles).
- If an active meeting exists: the full `content_html` is rendered inline using `dangerouslySetInnerHTML` on web and `WebView` with a full HTML shell on native, with a **"Read and Sign below ↓"** prompt at the end of the content.
- The outer `ScrollView` handles all scrolling — the meeting div has no internal scroll.

**Fixed bottom bar:**
- If not yet read: "Scroll to the bottom to unlock sign-off" message.
- If read but not signed: **Sign Off** button → navigates to `/(appointed-person)/crane-meeting/sign?meeting_id={id}`.
- If already signed: **Signed ✓** badge (green, non-interactive).
...
File: `app/(appointed-person)/crane-meeting/setup.tsx`

The Set Up form is a single scrollable screen. On load, if an active meeting already exists, the form pre-fills from that meeting (edit mode). Otherwise, persistent fields are pre-filled from `crane_meeting_settings`.

After successful submission the screen navigates to `router.replace('/(appointed-person)/crane-meeting/')`.

#### Section 1 — Project, Date & Time *(Date & Time per-meeting; Project fields persistent)*
| Field | Type |
|---|---|
| Project | Text input (persistent) |
| Project No | Text input (persistent) |
| Meeting Date | Date picker (per-meeting) |
| Meeting Time | Time picker (per-meeting) |

#### Section 2 — Meeting Details *(all persistent, all multi-line textareas)*
| Field | Label |
|---|---|
| Review of previous meeting | REVIEW OF YESTERDAY'S / LAST WEEK'S LIFTING OPERATIONS |
| Incidents / Problems | INCIDENTS / PROBLEMS |
| Revised Methods | REVISED METHODS |
| Future Lifts | FUTURE LIFTS |
| Weather Forecast | Weather forecast |
| New Methods | NEW METHODS |
| Lifting Equipment and Accessories | LIFTING equipment and accessories |
| Any Other Business | ANY OTHER BUSINESS (Holiday notice, bulletins, and alerts) |

#### Section 3 — Date of Next Meeting *(persistent)*
| Field | Type |
|---|---|
| Date of Next Meeting | Date picker |

#### Section 4 — Creator Signature *(fresh each meeting)*
| Field | Type |
|---|---|
| Signature | Drawn signature canvas (embedded inline in the form) |

**Submit flow:**
1. Validate required fields (project, meeting date, meeting time, signature).
2. Upload signature PNG to `crane-meeting-signatures` bucket at `setup/{site_id}/{meeting_date}_creator.png` (upsert:true).
3. Call `buildCraneMeetingHtml(data)` from `lib/crane-meeting-template.ts` to assemble `content_html`.
4. INSERT or UPDATE `crane_meetings` record.
5. UPSERT `crane_meeting_settings` with the persistent fields (`onConflict: 'site_id'`).
6. Navigate to home screen.

---

### 11.6 Document Rendering

The meeting document is assembled as a single HTML string by `buildCraneMeetingHtml(data: CraneMeetingTemplateData)` in `lib/crane-meeting-template.ts`. This function is the **single source of truth** for all HTML structure in the meeting document.

Each multi-line textarea value is rendered as:
```html
<div class="freetext">${escapeHtml(value).replace(/\n/g, '<br>')}</div>
```
so newlines survive when displayed inline and are recoverable as `\n` when `<br>` tags are converted back for PDF rendering.

**Template sections** (in order):
1. Meeting header (project name, project no, date, time)
2. REVIEW OF YESTERDAY'S / LAST WEEK'S LIFTING OPERATIONS
3. INCIDENTS / PROBLEMS
4. REVISED METHODS
5. FUTURE LIFTS
6. Weather Forecast
7. NEW METHODS
8. LIFTING equipment and accessories
9. ANY OTHER BUSINESS
10. Date of Next Meeting

The assembled HTML is stored in `crane_meetings.content_html` and never regenerated at read time.

**Platform rendering:**
- **Web:** injected via `dangerouslySetInnerHTML={{ __html: meeting.content_html }}` inside a `<div>`. `CRANE_MEETING_CONTENT_STYLES` from `lib/crane-meeting-template.ts` is injected into the document `<head>` to style the `.crane-meeting-content` class.
- **Native:** wrapped in a full HTML shell with `CRANE_MEETING_NATIVE_STYLES` inlined in the `<style>` tag, rendered inside a `WebView` with `scrollEnabled={false}` (outer `ScrollView` handles scrolling).

**Scroll tracking:** same pattern as Daily Briefing — the outer `ScrollView`'s `onScroll` handler detects `layoutMeasurement.height + contentOffset.y >= contentSize.height - 50` and records the read. `myReadRef` is used to avoid React closure stale-state bugs. If the meeting content fits entirely on screen without scrolling, the read is marked immediately on render.

---

### 11.7 Read Tracking + Signing

**Read tracking:**
- Scroll-to-bottom on the outer `ScrollView` triggers an INSERT into `crane_meeting_reads(meeting_id, user_id)`.
- `setMyRead(true)` fires immediately on detection — the Sign Off button appears without waiting for the DB round-trip.
- Unique constraint `(meeting_id, user_id)` — duplicate inserts return `23505` which is silently ignored.
- `myReadRef` is used in the `onScroll` handler to avoid stale closure bugs.

**Signing:**
- Sign screen (`/crane-meeting/sign?meeting_id={id}`) receives `meeting_id` as a URL param.
- Same drawn signature canvas as other sign screens — HTML5 canvas on web, `react-native-signature-canvas` on native.
- Pen colour `#000000`, white background baked in via `destination-over` on web.
- Signature uploaded to `crane-meeting-signatures` bucket at `{meeting_id}/{user_id}.png` (upsert:true).
- `crane_meeting_signatures` row inserted with `full_name`, `role`, and `company`:
  - `company` for `subcontractor_admin`: resolved from their `subcontractor_id` in `profiles` → `subcontractors.name`.
  - `company` for all other roles: resolved from `profiles.company_id` → `companies.name`.
- Unique constraint `(meeting_id, user_id)` — if `23505` is returned, "Already Signed" is shown and the user is navigated back.
- On success: `Alert.alert('Thank You', 'Thank you for signing the crane meeting.')` then `router.replace('/(appointed-person)/crane-meeting/')`.
- `router.replace` is used so the user cannot navigate back to the sign screen.
...
The **Who Signed** button (AP/supervisor only) opens `/(appointed-person)/crane-meeting/attendance?meeting_id={id}` as a modal screen.

**Display:** Two sections — **Read** and **Signed** — each listing operative names with timestamps. Counts shown in section headers: `Read (n)` / `Signed (n)`.

**Real-time updates:** Supabase Realtime subscriptions on both `crane_meeting_reads` and `crane_meeting_signatures` tables, filtered to `meeting_id=eq.{meeting_id}`. All `.on()` calls are chained before the single `.subscribe()` call in one fluent chain — splitting into multiple `.subscribe()` calls causes missed events.

**Channel name uniqueness:** Channel names include `Date.now()` plus a random suffix to avoid React 18 Strict Mode channel reuse bug. Without unique channel names, the second mount of the component in Strict Mode errors with "cannot add postgres_changes callbacks after subscribe()".

**Generate PDF:** A button calls `callCraneMeetingGeneratePdf(meetingId)` from `lib/api.ts`. On success, navigates to `router.replace('/(appointed-person)/crane-meeting/')`.

**Visibility:** Only `appointed_person` and `crane_supervisor` see the Who Signed button and the Generate PDF button. Other roles do not have access to the attendance screen.

---

### 11.9 Auto-Archive + Reset (Friday)

**Auto-archive:** A `pg_cron` job (`crane-meeting-friday-auto-archive`) runs at 19:59 Friday (UTC) and calls the `crane-meeting-generate-pdf` Edge Function for all meetings with `status = 'active'`.

**Safety-net reset:** A second `pg_cron` job (`crane-meeting-friday-reset`) runs at 20:00 Friday (UTC) and force-archives any still-active meeting by setting `status = 'archived'` directly in SQL — a safety net in case the Edge Function call fails.

**Manual Generate:** AP or supervisor can tap **Generate** on the home screen toolbar at any time. This calls `callCraneMeetingGeneratePdf(meetingId)` in `lib/api.ts`, which invokes the `crane-meeting-generate-pdf` Edge Function via `supabase.functions.invoke`.

**Edge Function** (`supabase/functions/crane-meeting-generate-pdf/index.ts`):

The function uses `pdf-lib` (Deno-compatible) and generates a complete multi-page archive PDF. All text is passed through a `clean()` sanitiser — same encoding rules as Section 11.13.

**Page order:**

1. **Cover page** — navy header bar; document title (`Crane Meeting - {site_name} - {day_of_week} - {date}` using plain hyphens as separators); date and site name; creator name and role (real role fetched from `profiles.role`); project and project no; meeting time; total signatory count; generation timestamp.

2. **Full meeting content pages** — every section of `crane_meetings.content_html` is parsed and rendered with word-wrapping and automatic page breaks as needed. `<div class="freetext">` blocks are captured in addition to `<h1>/<h2>/<h3>/<p>/<li>/<td>/<th>`. `<br>` tags inside freetext blocks are converted to `\n` before rendering so line breaks survive as paragraph breaks in the PDF.

3. **Creator sign-off page** — heading "Meeting Created and Signed By"; creator's full name; real role label (fetched from `profiles.role`, not a hard-coded string); embedded creator signature image (200×60px) with adequate spacing below the label; date of meeting.
...
await adminClient.storage.from('crane-meeting-archive').upload(pdfPath, pdfBytes, { upsert: false })
```
Path: `{site_id}/{meeting_id}.pdf`. Always `remove()` then `upload()` — never upsert.

**Database update:** set `archive_pdf_url = pdfPath`, `status = 'archived'`, `archived_at = new Date().toISOString()`.

**Cron mode vs. single mode:** If called without a `meeting_id` body param, the function queries all active meetings and processes each one. If called with `meeting_id`, it processes only that meeting.

---

### 11.10 Database Schema

Run `supabase/crane_meeting_schema.sql` in the Supabase SQL Editor to create all tables and indexes.

#### `crane_meeting_settings`
One row per site. Created on first Set Up submission via UPSERT (`onConflict: 'site_id'`).

| column | type | notes |
|---|---|---|
| site_id | uuid PK → sites | One row per site |
| project | text | Persistent — pre-filled each meeting |
| project_no | text | Persistent — pre-filled each meeting |
| review_previous | text | Persistent meeting detail textarea |
| incidents_problems | text | Persistent meeting detail textarea |
| revised_methods | text | Persistent meeting detail textarea |
| future_lifts | text | Persistent meeting detail textarea |
| weather_forecast | text | Persistent meeting detail textarea |
| new_methods | text | Persistent meeting detail textarea |
| lifting_equipment | text | Persistent meeting detail textarea |
| any_other_business | text | Persistent meeting detail textarea |
| date_of_next_meeting | date | Persistent — pre-filled each meeting |
| updated_at | timestamptz | default now() |
| updated_by | uuid FK → profiles | nullable |

#### `crane_meetings`
One active meeting per site at a time, enforced by partial unique index.

| column | type | notes |
|---|---|---|
| id | uuid PK | gen_random_uuid() |
| site_id | uuid FK → sites | |
| project | text | Snapshot at time of submission |
| project_no | text | Snapshot at time of submission |
| meeting_date | date | Per-meeting |
| meeting_time | text | Per-meeting (e.g. "09:00") |
| review_previous | text | Snapshot of persistent field |
| incidents_problems | text | Snapshot of persistent field |
| revised_methods | text | Snapshot of persistent field |
| future_lifts | text | Snapshot of persistent field |
| weather_forecast | text | Snapshot of persistent field |
| new_methods | text | Snapshot of persistent field |
| lifting_equipment | text | Snapshot of persistent field |
| any_other_business | text | Snapshot of persistent field |
| date_of_next_meeting | date | Snapshot of persistent field |
| creator_signature_url | text | Storage path in `crane-meeting-signatures` bucket |
| content_html | text | Full assembled HTML — not null |
| archive_pdf_url | text | Storage path in `crane-meeting-archive` bucket — set on archive |
| status | text | CHECK IN ('active', 'archived', 'deleted') default 'active' |
| created_by | uuid FK → profiles | not null |
| created_at | timestamptz | default now() |
| archived_at | timestamptz | Set when status changes to 'archived' |

**Partial unique index:** `crane_meetings_site_active ON crane_meetings(site_id) WHERE (status = 'active')` — prevents two active meetings for the same site simultaneously.

> **Status values:** `active` = currently live meeting; `archived` = PDF generated, read-only; `deleted` = soft-deleted by AP or supervisor.

#### `crane_meeting_reads`
| column | type | notes |
|---|---|---|
| id | uuid PK | gen_random_uuid() |
| meeting_id | uuid FK → crane_meetings | |
| user_id | uuid FK → profiles | |
| read_at | timestamptz | default now() |

**UNIQUE constraint:** `(meeting_id, user_id)` — one read record per user per meeting.

#### `crane_meeting_signatures`
| column | type | notes |
|---|---|---|
| id | uuid PK | gen_random_uuid() |
| meeting_id | uuid FK → crane_meetings | |
...
| role | text | Denormalised from auth context at sign time |
| company | text | Subcontractor company name for subcontractor_admin; site company for all others |
| signature_image_url | text | Storage path in `crane-meeting-signatures` bucket |
| signed_at | timestamptz | default now() |

**UNIQUE constraint:** `(meeting_id, user_id)` — one signature record per user per meeting.

---

### 11.11 Required Supabase Setup

The following must be configured in the Supabase Dashboard **before** the Crane Meeting feature will work. None of these are applied automatically.

> **IMPORTANT — Edge Functions must be redeployed after every code change.** Editing the local file in `supabase/functions/crane-meeting-generate-pdf/index.ts` and pushing to GitHub does **NOT** update the live function. The PDF is generated by the deployed function on Supabase's servers. After any change you **MUST** redeploy via the Supabase Dashboard: Edge Functions → `crane-meeting-generate-pdf` → Code tab → paste the full updated code → **Deploy updates**. Verify the deployment took effect by checking the Logs tab for the version log line (e.g. `CRANE-MEETING-GENERATE-PDF: v2`). This same critical rule applies to **all** Edge Functions in this project: `create-user`, `extract-docx-text`, `generate-signoff`, `mewp-generate-pdf`, `daily-briefing-generate-pdf`, `crane-meeting-generate-pdf`, `rescue-kit-generate-pdf`. Local edits + git push do **NOT** update any live function.

#### Database Tables
Run `supabase/crane_meeting_schema.sql` in the SQL Editor. This creates all four tables, RLS policies, the partial unique index, and supporting indexes.
...
| Bucket | Size limit | Allowed MIME types |
|---|---|---|
| `crane-meeting-signatures` | 5 MB | `image/png` |
| `crane-meeting-archive` | 50 MB | `application/pdf` |
...
#### Edge Function Deployment
Deploy the archive PDF generator via the Supabase Dashboard (no CLI required):
1. Dashboard → Edge Functions → **Deploy a new function** → Via Editor
2. Set the function name to `crane-meeting-generate-pdf`
3. Paste the full contents of `supabase/functions/crane-meeting-generate-pdf/index.ts`
4. Click **Deploy**

The function requires `SUPABASE_SERVICE_ROLE_KEY` and `SUPABASE_URL` environment variables set in Dashboard → Edge Functions → Secrets.

#### pg_cron Extension
Enable in **Database → Extensions**. Then register both Friday schedules (SQL included at the bottom of `supabase/crane_meeting_schema.sql`):
```sql
-- Auto-archive at 19:59 Friday
select cron.schedule(
...
-- Safety-net reset at 20:00 Friday
select cron.schedule(
...
### 11.12 Archive Screen

Visible to **all roles**. Lists all archived crane meetings for the current site where `status = 'archived'` and `archive_pdf_url IS NOT NULL`, ordered most recent first.

**Each card displays:**
- **Title** in the format: `Crane Meeting - {site_name} - {day_of_week} - {date}`
  - `day_of_week` is the full English weekday name (e.g. Tuesday)
  - `date` is formatted as `DD MMM YYYY` (e.g. 03 Jun 2026)
  - Example: `Crane Meeting - London Wall - Tuesday - 03 Jun 2026`
- **Metadata:** signatory count and archived time (formatted as `DD MMM YYYY, HH:MM`)
- **View button** — generates a signed URL with a 1-hour expiry and opens the archive PDF (`window.open` on web, `Linking.openURL` on native)
- **Delete button** — visible only to `appointed_person` and `crane_supervisor` — soft-deletes the meeting by setting `status = 'deleted'`; never hard-deletes. A confirmation prompt is shown before deletion (platform-aware: `window.confirm` on web, `Alert.alert` on native). The card disappears from the list immediately on success.

The Archive screen is accessible to all roles via the **Archive** button on the home screen — no active meeting is required. Other roles (crane_operator, slinger_signaller, subcontractor_admin) have read-only access: they can view PDFs but the Delete button is not shown.

---

### 11.13 PDF Encoding and Rendering Rules

These rules apply to the `crane-meeting-generate-pdf` Edge Function and follow the same standards as Section 10.14.

#### Character Encoding (WinAnsi / Latin-1)

All text drawn to the PDF MUST pass through a `clean()` sanitiser before being passed to `drawText()`. See Section 10.14 for the full specification.

#### HTML Entity Decoding

Decode HTML entities from `content_html` before drawing. See Section 10.14 for the full entity mapping table.

#### Freetext Block Rendering (`<div class="freetext">`)

The meeting content uses `<div class="freetext">` blocks for all multi-line textarea values. The PDF parser must capture the text content of these divs in addition to `<h1>/<h2>/<h3>/<p>/<li>/<td>/<th>`. `<br>` tags inside freetext blocks must be converted to `\n` before rendering, so line breaks within a textarea value survive as paragraph breaks in the PDF.

#### HTML Table Rendering

Same as Section 10.14 — parse `<tr>`/`<td>`/`<th>` rows and render as side-by-side columns. Never stack table cells as individual lines.

---

## 12. Rescue Kit Checklist

---

### 12.1 Overview

Per-site weekly tower crane rescue kit verification accessed from inside Supervisor Checks. The `appointed_person` or `crane_supervisor` adds a rescue kit once with persistent details (location, secured status, serial numbers, harness counts, certificates, etc.) and signs a weekly check each week. Each weekly signing produces a versioned PDF (v1, v2, v3…) archived permanently. Every Monday at 00:00 local time the pending status rotates so the kit awaits the next weekly check. `appointed_person` and `crane_supervisor` only — no operatives or other roles can view, add, edit, sign, or delete rescue kit entries.

**Entry point:** A Rescue Kit Checklist card on `app/(appointed-person)/supervisor-checks.tsx`, visible only to `appointed_person` and `crane_supervisor`.
...
| Role | Permissions |
|---|---|
| `appointed_person` | Full access — add, edit, sign, re-sign, delete, view archive |
| `crane_supervisor` | Full access — add, edit, sign, re-sign, delete, view archive |
| All other roles | No visibility — no entry point shown on Supervisor Checks, RLS blocks queries |
...
| Screen | Path | Description |
|---|---|---|
| Home | `/(appointed-person)/rescue-kit/` | Page header, Pending Check this week section, Archive preview section |
| Add / Edit | `/(appointed-person)/rescue-kit/add` | 4-section form to add or edit a rescue kit |
| Detail / Sign | `/(appointed-person)/rescue-kit/detail` | Kit summary grid, embedded document preview, supervisor name + signature + Sign button |
| Archive | `/(appointed-person)/rescue-kit/archive` | List of all signed PDFs for the site |

All four routes are registered in `app/(appointed-person)/_layout.tsx`. Entry point is the **Rescue Kit Checklist** card on `app/(appointed-person)/supervisor-checks.tsx`.

---

### 12.4 Home Screen Layout

The home screen refreshes on focus via `useFocusEffect`.

**Page header:** "Rescue Kit Checklist" title with subtitle "Weekly per-site tower crane rescue kit verification. Each kit must be checked and signed every week."

**Pending Check this week section card** with **Add** button (top right). Shows two sub-areas:
- Kits awaiting this week's check (no badge) — tap row to go to Detail / Sign screen
- Kits already signed this week (✓ Signed badge) — tap row to view the latest archived PDF

Every kit row (in both sub-areas) has a **three-dots menu** with **Edit** and **Delete** actions, available regardless of signed status.

**Dropdown menu implementation:** The three-dots dropdown menu uses `position: fixed` with computed coordinates from `getBoundingClientRect()` so it escapes any parent `overflow: hidden` and section card clipping. The menu auto-flips upward when the row is near the bottom of the viewport to avoid being clipped by sections below. All menu interactions (trigger button and each item) call `e.stopPropagation()` and `e.preventDefault()` so taps do not bubble up to the row (which would otherwise open the document or navigate). A document-level click-outside handler closes the menu when tapping anywhere else.

**Archive section** — preview of recent signed PDFs with a "View All →" link to the full Archive screen.

**Empty states:**
- "All rescue kits have been checked this week." — when all kits are signed for the current week.
- "No rescue kits added yet. Tap Add to create the first one." — when no kits exist.

---

### 12.5 Add / Edit Form

File: `app/(appointed-person)/rescue-kit/add.tsx`

All fields visible in one scrollable modal. Values are persistent — they stay until manually edited. Editing a kit that has already been signed for the current week does **not** reset its `last_signed_week_start` — the kit stays "signed for this week" after edits.

#### Section 1 — Identification *(all required)*
| Field | Notes |
|---|---|
| Main Contractor Name | e.g. "Multiplex" |
| Project Name | e.g. "London Wall" |
| Serial Number on the seal | |

#### Section 2 — Location & Access
| Field | Notes |
|---|---|
| Location of Rescue Kit? | Free text |
| Is it secured? | Yes / No toggle |
| How is it secured? | Free text (key / Code) |
| Who has access to the key/Code? | Free text |
| Individual / Company specific serial number | Optional — internally stored as `plrk_number` column (legacy name, never shown to users as "PLRK") |

#### Section 3 — Kit Contents
| Field | Notes |
|---|---|
| Is the Stretcher in the bag? | Yes / No toggle |
| Is the pole in the bag? | Yes / No toggle |
| How many Harnesses are with the kit? | Number input |
| Are they still new in packaging or have they been used? | Toggle: "Still new" / "Been used" |
| What are the serial numbers of the harness? | Multi-line free text |

#### Section 4 — Certification & Seal
| Field | Notes |
|---|---|
| Are all certificates of conformity / thorough examination with the kit? (Include Expiry Date) | Free text |
| Is the box still sealed? | Yes / No toggle |
| If the box is unsealed are all the contents still in the box as listed on the certificate of conformity? | Yes / No / N/A toggle |

---

### 12.6 Kit Detail & Sign Screen

File: `app/(appointed-person)/rescue-kit/detail.tsx`

**Top:** 3-column grid summary displaying all kit field values.

**Middle:** Embedded preview of the formatted Tower Crane Rescue Kit Checklist document (matching the PDF layout), rendered inline.

**Bottom:** Supervisor Name field (pre-filled with current user's name) + drawn signature canvas + **Sign** button.

For kits already signed this week, a **Sign Again** button creates a new version (v2, v3…) without overwriting the previous PDF. Each signing increments `last_version_number` on the `rescue_kits` record and inserts a new row in `rescue_kit_signed_checks`.

---

### 12.7 PDF Format

The PDF is generated by the `rescue-kit-generate-pdf` Edge Function using `pdf-lib`.

**No Pro-lifting branding anywhere** — no logo, no "Pro-lifting" / "pro-lifting" / "Prolifting" / "PLUK" text anywhere in the document or PDF output.

**Document title** (centred): `{main_contractor} {project_name} - {serial_number}`
Example: `Multiplex London Wall - 613/110817`

**Subtitle** (centred): `Tower Crane Rescue Kit Checklist`

**Lift Supervisor + Site row table**

**Main checklist table:** 2 columns (Question / Answer), one row per field (14 fields total).

**Footer notes** (static text):
...
**Signature box:** Signature image, Job Title, Print Name, Date.

**Page footer:** Page 1 of 1 centred + document reference on the right.

**Document reference format:** `{main_contractor} {project_name} Rescue Kit Checklist v{N}` where N increments per signed check (v1, v2, v3…).

---

### 12.8 Weekly Rotation

- A kit appears in "Pending Check this week" if `last_signed_week_start IS NULL` OR `last_signed_week_start < this_week_monday_date` (computed from `date_trunc('week', now())`).
- A `pg_cron` job (`rescue-kit-weekly-rotation`) runs every Monday at 00:01 — `last_signed_week_start` is the source of truth, rotation is derived at query time from this column.
- Kit data persists indefinitely; only `last_signed_week_start` and `last_version_number` change on each weekly sign.
- Archived signed PDFs remain accessible forever.

---

### 12.9 Database Schema

Run `supabase/rescue_kit_schema.sql` in the Supabase SQL Editor to create all tables, indexes, and RLS policies.

#### `rescue_kits`
Master record with all kit details.
...
| serial_number | text | Seal serial number — not null |
| location | text | |
| is_secured | boolean | |
| secured_how | text | |
| access_who | text | |
| plrk_number | text | Displayed as "Individual / Company specific serial number" — legacy column name, never expose as "PLRK" |
| stretcher_in_bag | boolean | |
| pole_in_bag | boolean | |
| harness_count | integer | |
| harnesses_used | text | "Still new" or "Been used" |
| harness_serials | text | Multi-line |
| certificates_conformity | text | Free text including expiry date |
| box_sealed | boolean | |
| contents_in_box | text | "Yes" / "No" / "N/A" |
| last_signed_week_start | date | NULL = never signed; set to Monday of current week on each sign |
| last_version_number | integer | default 0; increments on each sign |
| is_deleted | boolean | default false — soft delete only |
| created_by | uuid FK → profiles | |
| created_at | timestamptz | default now() |
| updated_at | timestamptz | default now() |

#### `rescue_kit_signed_checks`
One row per signed weekly check.

| column | type | notes |
|---|---|---|
| id | uuid PK | gen_random_uuid() |
| kit_id | uuid FK → rescue_kits | |
| site_id | uuid FK → sites | |
| version_number | integer | Increments per kit (1, 2, 3…) |
| supervisor_name | text | Name entered at sign time |
| supervisor_id | uuid FK → profiles | |
| signature_url | text | Storage path in `rescue-kit-signatures` bucket |
| pdf_url | text | Storage path in `rescue-kit-archive` bucket |
| week_start_date | date | Monday of the week this check covers |
| signed_at | timestamptz | default now() |
| is_archived | boolean | default false — soft delete for archive screen |

**UNIQUE constraint:** `(kit_id, version_number)` — one PDF per version per kit.

> Soft-delete only. Editing a signed kit does not reset weekly status. Deleting a kit (`rescue_kits.is_deleted = true`) does **not** affect existing signed PDFs in the archive.

---

### 12.10 Required Supabase Setup

The following must be configured in the Supabase Dashboard **before** the Rescue Kit Checklist feature will work. None of these are applied automatically.

> **IMPORTANT — Edge Functions must be redeployed after every code change.** Editing the local file in `supabase/functions/rescue-kit-generate-pdf/index.ts` and pushing to GitHub does **NOT** update the live function. After any change you **MUST** redeploy via the Supabase Dashboard: Edge Functions → `rescue-kit-generate-pdf` → Code tab → paste the full updated code → **Deploy updates**. Verify the deployment took effect by checking the Logs tab for the version log line (e.g. `RESCUE-KIT-GENERATE-PDF: v2`). This same critical rule applies to **all** Edge Functions in this project: `create-user`, `extract-docx-text`, `generate-signoff`, `mewp-generate-pdf`, `daily-briefing-generate-pdf`, `crane-meeting-generate-pdf`, `rescue-kit-generate-pdf`. Local edits + git push do **NOT** update any live function.

#### Database Tables
Run `supabase/rescue_kit_schema.sql` in the SQL Editor. This creates both tables, RLS policies, and supporting indexes.
...
| Bucket | Size limit | Allowed MIME types |
|---|---|---|
| `rescue-kit-signatures` | 5 MB | `image/png` |
| `rescue-kit-archive` | 50 MB | `application/pdf` |
...
#### Edge Function Deployment
Deploy the PDF generator via the Supabase Dashboard (no CLI required):
1. Dashboard → Edge Functions → **Deploy a new function** → Via Editor
2. Set the function name to `rescue-kit-generate-pdf`
3. Paste the full contents of `supabase/functions/rescue-kit-generate-pdf/index.ts`
4. Click **Deploy**

The function requires `SUPABASE_SERVICE_ROLE_KEY` and `SUPABASE_URL` environment variables set in Dashboard → Edge Functions → Secrets.

> **⚠️ Deployment naming warning:** When deploying Edge Functions via the Supabase Dashboard, you must use the exact function name. Do **NOT** paste code into an existing unrelated function. To deploy correctly: Dashboard → Edge Functions → **Deploy a new function** → Via Editor → set the function name to `rescue-kit-generate-pdf` → paste code → Deploy.

#### pg_cron Extension
Enable in **Database → Extensions**. Then register the Monday rotation schedule (SQL included at the bottom of `supabase/rescue_kit_schema.sql`):
```sql
select cron.schedule(
  'rescue-kit-weekly-rotation',
  '1 0 * * 1',
  $$ -- last_signed_week_start is the source of truth.
     -- Rotation is derived at query time; this job is a heartbeat placeholder. $$
);
```

---

### 12.11 Archive Screen

Visible to `appointed_person` and `crane_supervisor` only. Lists all signed checks for the current site where `is_archived = false`, ordered most recent first.

**Each card displays:**
- **Title** in the format: `{main_contractor} {project_name} Rescue Kit Checklist v{N}`
  - Example: `Multiplex London Wall Rescue Kit Checklist v1`
- **Metadata:** signed date (formatted as `DD MMM YYYY, HH:MM`), supervisor name
- **View button** — generates a signed URL with a 1-hour expiry and opens the archive PDF (`window.open` on web, `Linking.openURL` on native)
- **Delete button** — sets `rescue_kit_signed_checks.is_archived = true`; never hard-deletes. A confirmation prompt is shown before deletion. The card disappears from the list immediately on success.

---

### 12.12 PDF Encoding Rules

These rules apply to the `rescue-kit-generate-pdf` Edge Function and follow the same standards as Section 10.14.

#### Character Encoding (WinAnsi / Latin-1)

All text drawn to the PDF MUST pass through a `clean()` sanitiser before being passed to `drawText()`. See Section 10.14 for the full specification.

**No Pro-lifting / PLRK / PLUK references** anywhere in the PDF output. The column `plrk_number` is stored internally but displayed to users as "Individual / Company specific serial number" — never surface the column name in the UI or PDF.

#### HTML Entity Decoding

Decode any HTML entities from stored text before drawing. See Section 10.14 for the full entity mapping table.

> **CRITICAL deployment reminder:** editing the local Edge Function file does NOT update the deployed function. After every code change, paste the full updated code into the Supabase Dashboard Code tab and click "Deploy updates". Verify by checking the Logs tab for the version log line.

---

### 12.13 Common UI Patterns and Gotchas

Hard-won fixes documented here to avoid re-solving the same problems.

#### Dropdown Menus Inside Scrollable Section Cards

Dropdown menus inside scrollable section cards must use `position: fixed` with coordinates from `getBoundingClientRect()`, not `position: absolute`. Parent cards often have `overflow: hidden` which clips an absolutely-positioned dropdown. The `position: fixed` approach with computed coordinates plus a high z-index (9999) reliably renders above all sibling sections.

```tsx
const rect = triggerRef.current.getBoundingClientRect();
setMenuPosition({ top: rect.bottom, left: rect.left });
// In render: <View style={{ position: 'fixed', top: menuPosition.top, left: menuPosition.left, zIndex: 9999 }}>
```

#### Stopping Event Propagation on Menu Items in Tappable Rows

Menu items inside a tappable row must explicitly stop event propagation on both the trigger button and each menu item:

```tsx
onPress={(e: any) => { e?.stopPropagation?.(); e?.preventDefault?.(); handleAction(); }}
```

Without this, tapping a menu item bubbles up to the row and triggers its tap handler (e.g. navigating to the detail screen when the intent was to tap Edit).

#### Click-Outside-to-Close

Implemented via `document.addEventListener('click', handler)` in a `useEffect` that runs only while the menu is open. Cleanup removes the listener on unmount or when the menu closes:
...
#### Auto-Flip Dropdown Direction

For rows near the bottom of the viewport, compute `spaceBelow = window.innerHeight - rect.bottom` and position the menu above the trigger if `spaceBelow < dropdownHeight`. This keeps the menu fully visible regardless of position in a long list:

```tsx
const spaceBelow = window.innerHeight - rect.bottom;
const top = spaceBelow < DROPDOWN_HEIGHT ? rect.top - DROPDOWN_HEIGHT : rect.bottom;
setMenuPosition({ top, left: rect.left });
```

---

## 13. MEWP Inventory Module
...
### 13.4 Thorough Examination & Status Logic
...
### 13.8 Public MEWP Page (QR/NFC Scan — no login required)
...
### 13.9 Screen Structure (inside lifting-app)
...
### 13.10 Supabase Tables (MEWP module)
...
### 13.12 Supabase Storage Buckets
...
### 13.13 Authentication (MEWP module)
...
### 13.17 Critical Workarounds (do not skip)
...
### 13.18 RLS Policy Summary (MEWP tables)
