...
export default function CraneMeetingAttendance() {
  ...
  useEffect(() => {
    ...
    // Chain both .on() calls before .subscribe() — splitting into multiple subscribe() causes missed events
    const channelName = `crane-meeting-attendance-${meeting_id}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`

    const channel = supabase
      .channel(channelName)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'crane_meeting_reads', filter: `meeting_id=eq.${meeting_id}` },
        () => fetchAttendance()
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'crane_meeting_signatures', filter: `meeting_id=eq.${meeting_id}` },
        () => fetchAttendance()
      )
      .subscribe()
  ...
  async function handleGeneratePdf() {
    ...
    const confirmed = Platform.OS === 'web'
      ? window.confirm('Generate archive PDF and close the active crane meeting? Once archived it cannot be edited or signed.')
      : await new Promise<boolean>(resolve => {
          Alert.alert(
            'Generate Archive PDF',
            'This will create the archive PDF and close the crane meeting. Continue?',
            [
    ...
    console.log('[CM-ATTENDANCE] Generating PDF for meeting:', meeting_id)
    setIsGenerating(true)

    const { error } = await callCraneMeetingGeneratePdf(meeting_id)
    setIsGenerating(false)

    if (error) {
      console.error('[CM-ATTENDANCE] Generate failed:', error)
      Alert.alert(
        'Generate Failed',
        `${error}\n\nCheck that the crane-meeting-generate-pdf Edge Function is deployed to Supabase.`,
      )
      return
    }

    Alert.alert('Done', 'Archive PDF generated. The crane meeting has been archived.')
  ...
  if (!meeting_id) {
    return (
      <ScreenWrapper edges={['bottom']}>
        {breadcrumb}
        <View style={styles.centered}>
          <Text style={styles.emptyText}>No active crane meeting.</Text>
        </View>
      </ScreenWrapper>
    )
  }
          ...
          {canManage && (
            <View style={styles.generateSection}>
              ...
              <Text style={styles.generateHint}>
                Archives the crane meeting and creates a PDF with all signatures.
              </Text>
            </View>
          )}
        </ScrollView>
