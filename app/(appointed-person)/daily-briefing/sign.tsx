...
// IMPORTANT: Before using this feature, create the Supabase Storage bucket:
//   "daily-briefing-signatures" → private, 5 MB, image/png
//   INSERT and SELECT policies for authenticated role
...
export default function SignDailyBriefing() {
  ...
  useEffect(() => {
    async function resolveCompany() {
      ...
      if (role === 'subcontractor_admin') {
        // subcontractor_id may not be in the typed Profile — fetch it directly
  ...
  async function handleConfirm(sig: string) {
    console.log('[SIGN-BRIEFING] handleConfirm called (native), profile.id:', profile?.id, 'briefing_id:', briefing_id)
    if (!profile?.id || !briefing_id) {
      console.error('[SIGN-BRIEFING] Missing profile.id or briefing_id — aborting')
      return
    }

    const base64Data = sig.replace(/^data:image\/png;base64,/, '')
    const storagePath = `${briefing_id}/${profile.id}.png`

    setIsSubmitting(true)
    console.log('[SIGN-BRIEFING] Uploading signature to:', storagePath)
    ...
    if (uploadError) {
      console.error('[SIGN-BRIEFING] Upload error:', uploadError)
      setIsSubmitting(false)
      Alert.alert('Upload Error', uploadError.message)
      return
    }

    console.log('[SIGN-BRIEFING] Upload successful, inserting signature record')
    ...
    if (insertError) {
      console.error('[SIGN-BRIEFING] Insert error:', insertError.code, insertError.message)
      if (insertError.code === '23505') {
        Alert.alert('Already Signed', 'You have already signed this briefing.')
        router.back()
        return
      }
    ...
    console.log('[SIGN-BRIEFING] Signature saved — showing thank-you alert')
    Alert.alert(
      'Thank You',
      'Thank you for signing the daily briefing.',
      [{ text: 'OK', onPress: () => router.replace('/(appointed-person)/daily-briefing/' as any) }],
    )
  }
  ...
  async function handleSubmitSignature() {
    console.log('[SIGN-BRIEFING] handleSubmitSignature, platform:', Platform.OS)
    console.log('[SIGN-BRIEFING] signatureBase64 present:', !!signatureBase64, 'profile.id:', profile?.id)
    ...
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        const path = `${briefing_id}/${profile!.id}.png`

        console.log('[SIGN-BRIEFING] Uploading to storage path:', path)
        const { error: uploadError } = await supabase.storage
          .from('daily-briefing-signatures')
        ...
        console.log('[SIGN-BRIEFING] Inserting signature record')
        const { error: insertError } = await supabase
          .from('daily_briefing_signatures')
          .insert({
            briefing_id,
        ...
        if (insertError) {
          if (insertError.code === '23505') {
            window.alert('You have already signed this briefing.')
            router.back()
            return
          }
        ...
        console.log('[SIGN-BRIEFING] Signature saved — navigating to briefing home')
        window.alert('Thank you for signing the daily briefing.')
        router.replace('/(appointed-person)/daily-briefing/' as any)
      } catch (err: any) {
        console.error('[SIGN-BRIEFING] Submit failed:', err)
      ...
      console.log('[SIGN-BRIEFING] Calling readSignature() on native ref')
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.noteCard}>
          <Text style={styles.noteText}>
            By signing, you confirm that you have read and understood today's daily briefing.
          </Text>
        </View>
